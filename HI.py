...
data = pd.read_csv('C:/Users/dell/Desktop/feature_40/HI_feature_train.csv')
main_cols = data.columns.tolist()
label = pd.read_csv('C:/Users/dell/Desktop/feature_40/HI_train_label.csv')
test = pd.read_csv('C:/Users/dell/Desktop/feature_40/HI_feature_val.csv')
target_test = pd.read_csv('C:/Users/dell/Desktop/feature_40/HI_val_label.csv')
...
svr_CV = SVC(probability=True,C=4.65,gamma=0.02,kernel='rbf',random_state=12,class_weight={0: 0.116, 1: 0.397})
...
for i in range(5):
    ...
    print(f"fold:{i}")
    train_index = pd.read_csv('fold3_new'+str(i)+'/'+'train_index.csv',header=None).iloc[:, 0].tolist()
    test_index = pd.read_csv('fold3_new' +str(i) + '/' + 'test_index.csv',header=None).iloc[:, 0].tolist()
    ...
    drop = [0,5,7,9,24,30,31,39,40,42,46]
...
for i in range(5):
    ...
    logistic_model = LogisticRegression(C=30,max_iter=20, random_state=3)
...
for i in range(5):
    ...
    rf_model = RandomForestClassifier(n_estimators=5, random_state=5)
...
for i in range(5):
    ...
    plt.tight_layout()
    plt.savefig(f"{save_dir}/Gradient Boosting_fold_external_{i+1}.png")  # 保存外部验证集的混淆矩阵图像
    plt.close()  # 关闭当前图形
...
for i in range(5):
    ...
    knn_model = KNeighborsClassifier(n_neighbors=1,p=3)
