...
data = pd.read_csv('C:/Users/dell/Desktop/feature_40/IA_feature_train.csv')
main_cols = data.columns.tolist()
label = pd.read_csv('C:/Users/dell/Desktop/feature_40/IA_train_label.csv')
test = pd.read_csv('C:/Users/dell/Desktop/feature_40/IA_feature_val.csv')
target_test = pd.read_csv('C:/Users/dell/Desktop/feature_40/IA_val_label.csv')
...
svr_CV = SVC(probability=True,C=3.33,gamma=0.06,kernel='rbf',random_state=17,class_weight={0: 0.828, 1: 0.124})
...
for i in range(5):
    ...
    print(f"fold:{i}")
    train_index = pd.read_csv('fold2_new'+str(i)+'/'+'train_index.csv',header=None).iloc[:, 0].tolist()
    test_index = pd.read_csv('fold2_new' +str(i) + '/' + 'test_index.csv',header=None).iloc[:, 0].tolist()
    ...
    drop = [1,3,7,13,17,25,27,31,34,35,37,40,43,44,56,57,63,65,67,68,69]
...
for i in range(5):
    ...
    logistic_model = LogisticRegression(C=10,max_iter=10, random_state=5)
